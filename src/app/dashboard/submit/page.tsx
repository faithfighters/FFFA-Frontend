...
import { Upload, CheckCircle, Target, Send } from 'lucide-react';
...
export default function DashboardSubmitPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (causeMode === 'new' && (!causeName || !causeCategory)) { setError('Please fill in cause name and category.'); return; }
        if (causeMode === 'existing' && !selectedExistingCauseId) { setError('Please select an existing cause.'); return; }
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className={styles.formCard}>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <div
                            ...
                            style={{ borderColor: isDragging ? '#dc2626' : file ? '#16a34a' : undefined, background: isDragging ? '#fff5f6' : file ? '#f0fdf4' : undefined }}
                        ...
                        >
                            <span className={styles.dropzoneIcon}>
                                <Upload size={28} color={file ? '#16a34a' : isDragging ? '#dc2626' : '#94a3b8'} />
                            </span>
                            <p className={styles.dropzoneText} style={{ color: file ? '#16a34a' : undefined, fontWeight: file ? 600 : undefined }}>
                                {fileName || 'Drag & drop your video here, or click to browse'}
                            </p>
                            <p style={{ fontSize: '12px', color: '#94a3b8', margin: '4px 0 0' }}>MP4, MOV, WebM — max 1 minute · up to 200MB</p>
                        </div>
                ...
                {/* ── Section 2: Cause ── */}
                <div className={styles.formCard}>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Target size={18} color="#dc2626" /> Linked Cause <span style={{ fontWeight: 500, color: '#94a3b8', fontSize: '13px' }}>(members vote on this)</span></h3>
                ...
                {loading && step && (
                    <p style={{ fontSize: '13px', color: '#64748b', fontWeight: 600, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><span>{step}</span></p>
                )}
