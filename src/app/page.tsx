...
export default function Home() {
  ...
  const [showImpactVideo, setShowImpactVideo] = useState(false);
  ...
  return (
    <main className={styles.main}>
            ...
            <motion.div variants={fadeInUp} style={{ display: 'flex', gap: '12px', marginTop: '48px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '50px', padding: '8px 16px' }}>
                  <span style={{ color: '#F8C38F' }}>{stat.icon}</span>
              ...
              <div className={styles.videoCard}>
                ...
                <div className={styles.videoOverlay}>
                  <motion.div
                    ...
                    onClick={() => setShowImpactVideo(true)}
                  >
          ...
          <div className={styles.reelGrid}>
            ...
            <div className={styles.reelCarouselArea}>
              <motion.button
                ...
                whileHover={{ scale: 1.1, backgroundColor: '#E7421B' }}
                whileTap={{ scale: 0.93 }}
                onClick={() => {
                  ...
                  ? [
                    ...
                    >
                      <div className={styles.reelThumbWrapper}>
                        ...
                        <div className={styles.reelBottomInfo}>
                          <p className={styles.reelCardTitle}>{reel.title}</p>
                          <div className={styles.reelUploaderRow}>By Community Member</div>
                          <div className={styles.reelProgressWrapper}>
                            <div className={styles.reelProgressBg}>
                              ...
                              >
                                <Flame size={12} color="#f97316" className={styles.fireEmoji} />
                              </motion.div>
                            </div>
                          </div>
                          <div className={styles.reelMetaRow}>
                            <div className={styles.reelVoteStat}>
                              <Flame size={12} color="#ffffff" /> {reel.progress}% funded
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  : reelVideos.map((video, idx) => (
                    ...
                    >
                      <div className={styles.reelThumbWrapper} style={{ position: 'relative', overflow: 'hidden' }}>
                        ...
                        {/* Dark gradient scrim so overlaid text stays legible */}
                        <div className={styles.reelGradientOverlay} />
                        ...
                        {/* Bottom overlay: title, uploader, votes, progress */}
                        <div className={styles.reelBottomInfo}>
                          <p className={styles.reelCardTitle}>{video.title}</p>
                          <div className={styles.reelUploaderRow}>By {video.authorName}</div>
                          <div className={styles.reelProgressWrapper}>
                            <div className={styles.reelProgressBg}>
                              <motion.div
                                className={styles.reelProgressFill}
                                initial={{ width: 0 }}
                                whileInView={{ width: video.requiredVotes ? `${Math.min(100, Math.round(((video.voteCount ?? 0) / video.requiredVotes) * 100))}%` : '0%' }}
                                viewport={{ once: true }}
                          ...
                          <div className={styles.reelMetaRow}>
                            ...
                            {video.requiredVotes ? (
                              <div className={styles.reelVotePct}>
          ...
          >
            ...
            <div style={{ position: 'relative', zIndex: 1, maxWidth: '520px' }}>
              <motion.div variants={fadeInUp} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                <div style={{ width: '36px', height: '2px', background: 'linear-gradient(90deg, #F8C38F 0%, #E7421B 100%)' }} />
                <span style={{ color: '#F8C38F', fontSize: '12px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px' }}>Official Store</span>
              </motion.div>
              <motion.h2 variants={fadeInUp} style={{ fontFamily: 'var(--font-roboto)', fontWeight: 900, fontSize: 'clamp(32px,4vw,52px)', lineHeight: 1.05, textTransform: 'uppercase', color: 'white', marginBottom: '20px' }}>
                GEAR THAT{' '}
                <span style={{ background: 'linear-gradient(135deg, #F8C38F 0%, #E7421B 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>GIVES BACK</span>
              </motion.h2>
              ...
              <motion.div variants={fadeInUp} style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                <motion.div whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.97 }}>
                  <a href="https://shop.faithfightersforamerica.com/" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 32px', background: 'linear-gradient(90deg, #F8C38F 0%, #E7421B 100%)', color: 'white', borderRadius: '50px', fontWeight: 800, fontSize: '14px', textDecoration: 'none', boxShadow: '0 8px 24px rgba(231,66,27,0.4)', fontFamily: 'var(--font-roboto)', letterSpacing: '0.5px', textTransform: 'uppercase' }}>
            ...
            <motion.div variants={fadeInRight} style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: 'repeat(3, minmax(100px, 130px))', gap: '16px', width: '100%', maxWidth: '420px', margin: '0 auto' }}>
              {[
                { label: 'Faith Hoodie', price: '$49', icon: <Shirt size={44} color="rgba(255,255,255,0.7)" />, color: 'rgba(255,255,255,0.05)', badge: 'NEW' },
                { label: 'FFFA Cap', price: '$29', icon: <ShoppingBag size={44} color="rgba(255,255,255,0.7)" />, color: 'rgba(248,195,143,0.07)', badge: 'HOT' },
                { label: 'Unity Tee', price: '$35', icon: <Shirt size={44} color="rgba(255,255,255,0.7)" />, color: 'rgba(255,255,255,0.03)', badge: null },
              ].map((item, i) => (
                <motion.div
                  key={i}
                  whileHover={{ y: -10, boxShadow: '0 24px 48px rgba(0,0,0,0.4)' }}
                  style={{
                    background: item.color, borderRadius: '20px', padding: '28px 16px 20px',
                ...
                >
                  {item.badge && (
                    <div style={{ position: 'absolute', top: '10px', right: '10px', background: item.badge === 'NEW' ? '#10b981' : 'linear-gradient(90deg, #F8C38F 0%, #E7421B 100%)', color: 'white', fontSize: '12px', fontWeight: 800, padding: '2px 7px', borderRadius: '50px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{item.badge}</div>
                  )}
                  ...
                  <div style={{ background: 'linear-gradient(135deg, #F8C38F 0%, #E7421B 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', fontWeight: 900, fontSize: '16px' }}>{item.price}</div>
      ...
      {/* Impact video — placeholder until the final production video is delivered */}
      {showImpactVideo && (
        <div
          onClick={() => setShowImpactVideo(false)}
          style={{
