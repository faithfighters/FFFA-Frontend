...
interface AuthContextType {
    ...
    register: (name: string, email: string, password: string, plan?: 'faith_builder' | 'faith_hero' | 'faith_fighter', userType?: 'donor' | 'recipient') => Promise<{ success: boolean; error?: string }>;
    ...
    const register = async (
        ...
        try {
            const res = await fetch('/api/auth/register', {
                ...
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({ name, email, password, ...(plan ? { plan } : {}), ...(userType ? { userType } : {}) }),
            });
            const data = await res.json();
            if (res.ok) {
                return { success: true };
