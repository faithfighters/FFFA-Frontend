...
interface AuthContextType {
    ...
    register: (name: string, email: string, password: string, plan?: 'faith_builder' | 'faith_hero' | 'faith_fighter', userType?: 'donor' | 'recipient', intake?: { phone?: string; city?: string; helpType?: string; message?: string }) => Promise<{ success: boolean; error?: string }>;
    verifyRegisterOtp: (email: string, code: string) => Promise<{ success: boolean; error?: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
        ...
        try {
            const res = await fetch('/api/auth/register', {
                ...
                body: JSON.stringify({
                    ...
                    ...(intake?.helpType ? { helpType: intake.helpType } : {}),
                    ...(intake?.message ? { message: intake.message } : {}),
                }),
            });
            const data = await res.json();
            if (res.ok) {
    ...
    const verifyRegisterOtp = async (email: string, code: string): Promise<{ success: boolean; error?: string }> => {
        try {
